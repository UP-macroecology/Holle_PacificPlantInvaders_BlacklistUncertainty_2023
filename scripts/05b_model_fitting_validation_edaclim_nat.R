...
foreach(sp = study_species, .packages = c("mgcv", "randomForest", "gbm", "dismo", "PresenceAbsence", "ecospat")) %dopar% { # Start the loop over all species
  try({
    ...
    # with models does not exist yet
      ...
      absences_indices <- which(species_occ_edaclim_native$occ == 0)
      ...
      m_rf_edaclim_native <- lapply(1:10, FUN=function(i){sp_train <- species_occ_edaclim_native[c(presences_indices, which(species_occ_edaclim_native$abs_index == i)),]; print(i); randomForest(as.formula(paste('occ~',paste(pred_sel_edaclim_native, collapse='+'))), 
      ...
      m_brt_edaclim_native <- lapply(1:10, FUN=function(i) {
        ...
        LR <- 0.01;
        while(opt.LR){
          m.brt <- try(gbm.step(data = species_occ_edaclim_native[c(presences_indices, which(species_occ_edaclim_native$abs_index == i)),], gbm.x = pred_sel_edaclim_native, gbm.y = "occ", family = 'bernoulli', tree.complexity = 2, bag.fraction = 0.75, learning.rate = LR, verbose=F, plot.main=F))
...
# 2. Model validation ---------------------------------------------------------- 
    ...
    # with validation files does not exist yet
      ...
      crossval_folds <- dismo::kfold(seq_len(nrow(species_occ_edaclim_native)), k = kfolds)
      ...
      m_glm_preds_cv <- rep(NA, nrow(species_occ_edaclim_native))
      for(i in seq_len(kfolds)){
        print(i)
        cv_train <- species_occ_edaclim_native[crossval_folds!=i,]
        cv_test <- species_occ_edaclim_native[crossval_folds==i,]
        cv_weights <- weights[crossval_folds!=i]
        
        cv_glm <- update(m_glm_edaclim_native, data=cv_train, weights=cv_weights)
        m_glm_preds_cv[crossval_folds==i] <- predict(cv_glm, cv_test, type='response')
      }
      
      # Start preparations for model validation
      m_glm_preds_cv_edaclim_native_presences <- m_glm_preds_cv[presences_indices] # Just retain the predictions of the presences based on indices
      ...
      thresh_tenthPer <- quantile(m_glm_preds_cv_edaclim_native_presences, probs = 0.10)
      
      # Calculate performance measures based on different thresholds
      m_glm_perf_cv_maxTSS <- evalSDM(species_occ_edaclim_native$occ, m_glm_preds_cv, thresh.method='MaxSens+Spec', weigths = weights)
      m_glm_perf_cv_meanProb <- evalSDM(species_occ_edaclim_native$occ, m_glm_preds_cv, thresh.method='MeanProb', weigths = weights)
      m_glm_perf_cv_tenthPer <- evalSDM(species_occ_edaclim_native$occ, m_glm_preds_cv, thresh = thresh_tenthPer, weigths = weights)
      
      # Calculate the Boyce index additionally
      boyce_index_glm_edaclim_native <- ecospat.boyce(fit = m_glm_preds_cv, obs = m_glm_preds_cv_edaclim_native_presences,
      ...
      m_glm_perf_cv_maxTSS$Boyce <- boyce_index_glm_edaclim_native
      m_glm_perf_cv_meanProb$Boyce <- boyce_index_glm_edaclim_native
      m_glm_perf_cv_tenthPer$Boyce <- boyce_index_glm_edaclim_native
      ...
      m_gam_preds_cv <- rep(NA, nrow(species_occ_edaclim_native))
      for(i in seq_len(kfolds)){
        print(i)
        cv_train <- species_occ_edaclim_native[crossval_folds!=i,]
        cv_test <- species_occ_edaclim_native[crossval_folds==i,]
        ...
        cv_gam <- update(m_gam_edaclim_native, data=cv_train, weights=cv_weights)
        m_gam_preds_cv[crossval_folds==i] <- predict(cv_gam, cv_test, type='response')
      }
      
      # Start preparations for model validation
      m_gam_preds_cv_edaclim_native_presences <- m_gam_preds_cv[presences_indices] # Just retain the predictions of the presences based on indices
      
      
      # Calculate the 10th percentile of cross-validated prediction probabilities
      # of presence locations (10th percentile of training presences)
      thresh_tenthPer <- quantile(m_gam_preds_cv_edaclim_native_presences, probs = 0.10)
      
      # Calculate performance measures based on different thresholds
      m_gam_perf_cv_maxTSS <- evalSDM(species_occ_edaclim_native$occ, m_gam_preds_cv, thresh.method='MaxSens+Spec', weigths = weights)
      m_gam_perf_cv_meanProb <- evalSDM(species_occ_edaclim_native$occ, m_gam_preds_cv, thresh.method='MeanProb', weigths = weights)
      m_gam_perf_cv_tenthPer <- evalSDM(species_occ_edaclim_native$occ, m_gam_preds_cv, thresh = thresh_tenthPer, weigths = weights)
      
      # Calculate the Boyce index additionally
      boyce_index_gam_edaclim_native <- ecospat.boyce(fit = m_gam_preds_cv, obs = m_gam_preds_cv_edaclim_native_presences,
                                                      nclass=0, window.w="default", res=100, PEplot = FALSE, 
                                                      rm.duplicate = TRUE, method = 'kendall')
      ...
      # Add the Boyce index to the performance metrics data frame
      m_gam_perf_cv_maxTSS$Boyce <- boyce_index_gam_edaclim_native
      m_gam_perf_cv_meanProb$Boyce <- boyce_index_gam_edaclim_native
      m_gam_perf_cv_tenthPer$Boyce <- boyce_index_gam_edaclim_native
      ...
      m_rf_predmat_cv <- matrix(nrow=nrow(species_occ_edaclim_native), ncol=10)
      for(i in seq_len(kfolds)){ # loop over k folds
        ...
        for (m in 1:10) { # loop over 10 models
          ...
          dat_tenth <- c(presences_indices, which(species_occ_edaclim_native$abs_index == m))
          
          cv_train <- species_occ_edaclim_native[hold_in[hold_in %in% dat_tenth],]
          cv_test <- species_occ_edaclim_native[hold_out,]
          
          cv_rf <- update(m_rf_edaclim_native[[m]], data=cv_train)
      ...
      # Start preparations for model validation
      m_rf_preds_cv_edaclim_native_presences <- m_rf_preds_cv[presences_indices] # Just retain the predictions of the presences based on indices
      
      # Calculate the 10th percentile of cross-validated prediction probabilities
      # of presence locations (10th percentile of training presences)
      thresh_tenthPer <- quantile(m_rf_preds_cv_edaclim_native_presences, probs = 0.10)
      
      # Calculate performance measures based on different thresholds
      m_rf_perf_cv_maxTSS <- evalSDM(species_occ_edaclim_native$occ, m_rf_preds_cv, thresh.method='MaxSens+Spec', weigths = weights)
      m_rf_perf_cv_meanProb <- evalSDM(species_occ_edaclim_native$occ, m_rf_preds_cv, thresh.method='MeanProb', weigths = weights)
      m_rf_perf_cv_tenthPer <- evalSDM(species_occ_edaclim_native$occ, m_rf_preds_cv, thresh = thresh_tenthPer, weigths = weights)
      
      # Calculate the Boyce index additionally
      boyce_index_rf_edaclim_native <- ecospat.boyce(fit = m_rf_preds_cv, obs = m_rf_preds_cv_edaclim_native_presences,
                                                     nclass=0, window.w="default", res=100, PEplot = FALSE, 
                                                     rm.duplicate = TRUE, method = 'kendall')
      
      boyce_index_rf_edaclim_native <- boyce_index_rf_edaclim_native$cor # Extract correlation value (Boyce index)
      
      # Add the Boyce index to the performance metrics data frame
      m_rf_perf_cv_maxTSS$Boyce <- boyce_index_rf_edaclim_native
      m_rf_perf_cv_meanProb$Boyce <- boyce_index_rf_edaclim_native
      m_rf_perf_cv_tenthPer$Boyce <- boyce_index_rf_edaclim_native
      ...
      m_brt_predmat_cv <- matrix(nrow=nrow(species_occ_edaclim_native), ncol=10)
      for(i in seq_len(kfolds)){ # loop over k folds
        ...
        for (m in 1:10) { # loop over 10 models
          ...
          cv_train <- species_occ_edaclim_native[hold_in[hold_in %in% dat_tenth],]
          names(cv_train)[names(cv_train)=='occ'] <- m_brt_edaclim_native[[m]]$response.name
          cv_test <- species_occ_edaclim_native[hold_out,]
          
          cv_brt <- gbm::gbm(m_brt_edaclim_native[[m]]$call, 'bernoulli', data = cv_train[,c( m_brt_edaclim_native[[m]]$response.name, pred_sel_edaclim_native)], 
                             n.trees=m_brt_edaclim_native[[m]]$gbm.call$best.trees, 
                             shrinkage=m_brt_edaclim_native[[m]]$gbm.call$learning.rate, 
                             bag.fraction=m_brt_edaclim_native[[m]]$gbm.call$bag.fraction, 
                             interaction.depth=m_brt_edaclim_native[[m]]$gbm.call$tree.complexity)
          m_brt_predmat_cv[hold_out,m] <- predict(cv_brt, cv_test, type='response', n.trees=m_brt_edaclim_native[[m]]$gbm.call$best.trees)
        }
      }
      ...
      # Start preparations for model validation
      m_brt_preds_cv_edaclim_native_presences <- m_brt_preds_cv[presences_indices] # Just retain the predictions of the presences based on indices
      
      
      # Calculate the 10th percentile of cross-validated prediction probabilities
      # of presence locations (10th percentile of training presences)
      thresh_tenthPer <- quantile(m_brt_preds_cv_edaclim_native_presences, probs = 0.10)
      
      # Calculate performance measures based on different thresholds
      m_brt_perf_cv_maxTSS <- evalSDM(species_occ_edaclim_native$occ, m_brt_preds_cv, thresh.method='MaxSens+Spec', weigths = weights)
      m_brt_perf_cv_meanProb <- evalSDM(species_occ_edaclim_native$occ, m_brt_preds_cv, thresh.method='MeanProb', weigths = weights)
      m_brt_perf_cv_tenthPer <- evalSDM(species_occ_edaclim_native$occ, m_brt_preds_cv, thresh = thresh_tenthPer, weigths = weights)
      
      # Calculate the Boyce index additionally
      boyce_index_brt_edaclim_native <- ecospat.boyce(fit = m_brt_preds_cv, obs = m_brt_preds_cv_edaclim_native_presences,
                                                      nclass=0, window.w="default", res=100, PEplot = FALSE, 
                                                      rm.duplicate = TRUE, method = 'kendall')
      
      boyce_index_brt_edaclim_native <- boyce_index_brt_edaclim_native$cor # Extract correlation value (Boyce index)
      
      # Add the Boyce index to the performance metrics data frame
      m_brt_perf_cv_maxTSS$Boyce <- boyce_index_brt_edaclim_native
      m_brt_perf_cv_meanProb$Boyce <- boyce_index_brt_edaclim_native
      m_brt_perf_cv_tenthPer$Boyce <- boyce_index_brt_edaclim_native
      
      
      
      # (e) Summarize performance metrics  -------------------------------------
      # for each algorithm and threshold
      comp_perf_edaclim_native_maxTSS <- rbind(glm = m_glm_perf_cv_maxTSS, gam = m_gam_perf_cv_maxTSS, rf = m_rf_perf_cv_maxTSS, brt = m_brt_perf_cv_maxTSS)
      comp_perf_edaclim_native_maxTSS <- data.frame(alg=row.names(comp_perf_edaclim_native_maxTSS),comp_perf_edaclim_native_maxTSS) # Add a column containing the names of the algorithm
      
      comp_perf_edaclim_native_meanProb <- rbind(glm = m_glm_perf_cv_meanProb, gam = m_gam_perf_cv_meanProb, rf = m_rf_perf_cv_meanProb, brt = m_brt_perf_cv_meanProb)
      comp_perf_edaclim_native_meanProb <- data.frame(alg=row.names(comp_perf_edaclim_native_meanProb),comp_perf_edaclim_native_meanProb)
      
      comp_perf_edaclim_native_tenthPer <- rbind(glm = m_glm_perf_cv_tenthPer, gam = m_gam_perf_cv_tenthPer, rf = m_rf_perf_cv_tenthPer, brt = m_brt_perf_cv_tenthPer)
      comp_perf_edaclim_native_tenthPer <- data.frame(alg=row.names(comp_perf_edaclim_native_tenthPer),comp_perf_edaclim_native_tenthPer)
      ...
      preds_cv_all_edaclim_native <- data.frame(glm = m_glm_preds_cv, gam = m_gam_preds_cv,  rf = m_rf_preds_cv, brt = m_brt_preds_cv)
      
      # Calculate cross-validated ensemble predictions (mean)
      preds_cv_ens_edaclim_native <- rowMeans(preds_cv_all_edaclim_native) # using the mean
      
      # Start preparations for model validation
      preds_cv_ens_edaclim_native_presences <- preds_cv_ens_edaclim_native[presences_indices] # Just retain the predictions of the presences based on indices
      
      
      # Calculate the 10th percentile of cross-validated prediction probabilities
      # of presence locations (10th percentile of training presences)
      thresh_tenthPer <- quantile(preds_cv_ens_edaclim_native_presences, probs = 0.10)
      
      # Calculate ensemble performance metrics based on different thresholds
      ensemble_perf_edaclim_native_maxTSS <- evalSDM(species_occ_edaclim_native$occ, preds_cv_ens_edaclim_native, thresh.method = 'MaxSens+Spec', weigths = weights)
      ensemble_perf_edaclim_native_meanProb <- evalSDM(species_occ_edaclim_native$occ, preds_cv_ens_edaclim_native, thresh.method = 'MeanProb', weigths = weights)
      ensemble_perf_edaclim_native_tenthPer <- evalSDM(species_occ_edaclim_native$occ, preds_cv_ens_edaclim_native, thresh = thresh_tenthPer, weigths = weights)
      
      
      # Calculate the Boyce index of the ensemble
      boyce_index_ens_edaclim_native <- ecospat.boyce(fit = preds_cv_ens_edaclim_native, obs = preds_cv_ens_edaclim_native_presences,
                                                      nclass=0, window.w="default", res=100, PEplot = FALSE, 
                                                      rm.duplicate = TRUE, method = 'kendall')
      
      boyce_index_ens_edaclim_native <- boyce_index_ens_edaclim_native$cor # Extract correlation value (Boyce index)
      
      # Add the Boyce index to the performance metrics data frame
      ensemble_perf_edaclim_native_maxTSS$Boyce <- boyce_index_ens_edaclim_native
      ensemble_perf_edaclim_native_meanProb$Boyce <- boyce_index_ens_edaclim_native
      ensemble_perf_edaclim_native_tenthPer$Boyce <- boyce_index_ens_edaclim_native
      
      # Comparing cross-validated ensemble model performance across thresholds:
      ensemble_perf_edaclim_native <- rbind(maxTSS = ensemble_perf_edaclim_native_maxTSS,
                                            meanProb = ensemble_perf_edaclim_native_meanProb,
                                            tenthPer = ensemble_perf_edaclim_native_tenthPer)
