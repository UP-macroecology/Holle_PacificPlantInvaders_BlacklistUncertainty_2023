...
# ---------------------------------------------------------------------- #
#             09c. Unrealized colonization potential based on            #
...
# Load needed objects
load("output_data/model_predictions/global/clim/islandgroups_results_clim_global.RData") # Data frame with prediction results of all species based on 49 island groups
...
unreal_col_pot_clim_global <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(unreal_col_pot_clim_global) <- c("unrealized_col_pot", "predictor_set", "species")

# Solely use the predictions of ensemble models
islandgroups_results_clim_global_ensemble <- subset(islandgroups_results_clim_global, islandgroups_results_clim_global$algorithm == "Ensemble")


for (sp in study_species) { # Start of the loop over all study species
  
  # Create a data frame to store the results per species
  unreal_col_pot_clim_global_species <- data.frame(matrix(ncol = 7, nrow = 0))
  colnames(unreal_col_pot_clim_global_species) <- c("islandgroup", "a", "b", "c", "d", "predictor_set", "species")
  
  # Subset the prediction data frame by each species
  subset_species <- subset(islandgroups_results_clim_global_ensemble, islandgroups_results_clim_global_ensemble$species == sp)
  ...
  for (i in islandgroup_climate) { # Start of the loop over all island groups
    ...
    if (check_observation == 1 & check_prediction == 1) { results_col_pot <- c(i, 1, 0, 0, 0, 3, sp)
    } else if (check_observation == 0 & check_prediction == 0) { results_col_pot <- c(i, 0, 0, 0, 1, 3, sp)
    } else if (check_observation == 0 & check_prediction == 1) { results_col_pot <- c(i, 0, 1, 0, 0, 3, sp)
    } else if (check_observation == 1 & check_prediction == 0) { results_col_pot <- c(i, 0, 0, 1, 0, 3, sp) 
    }
    
    # Add the vector containing the results to the data frame
    unreal_col_pot_clim_global_species <- rbind(unreal_col_pot_clim_global_species, results_col_pot)
    
    # Make sure the column names are correct
    colnames(unreal_col_pot_clim_global_species) <- c("islandgroup", "a", "b", "c", "d", "predictor_set", "species")
    
    # Save the data frame for each species
    save(unreal_col_pot_clim_global_species, file = paste0("output_data/unrealized_col_pot/global/clim/unreal_col_pot_clim_global_species_",sp,".RData"))
  ...
  col_pot_a <- sum(as.numeric(unreal_col_pot_clim_global_species$a)) # a
  col_pot_b <- sum(as.numeric(unreal_col_pot_clim_global_species$b)) # b
  col_pot_c <- sum(as.numeric(unreal_col_pot_clim_global_species$c)) # c
  col_pot_d <- sum(as.numeric(unreal_col_pot_clim_global_species$d)) # d
  
  false_positives <- round(col_pot_b/(col_pot_b + col_pot_d), 2)
  
  # Create a vector with results
  results_unreal_col_pot <- c(false_positives, 3, sp)
  
  # Add result to the data frame
  unreal_col_pot_clim_global <- rbind(unreal_col_pot_clim_global, results_unreal_col_pot)
...
save(unreal_col_pot_clim_global, file = "output_data/unrealized_col_pot/global/clim/unreal_col_pot_clim_global.RData")
...
# Load needed objects
load("output_data/model_predictions/global/clim_comp/islandgroups_results_clim_global_comp.RData") # Data frame with prediction results of all species based on 25 island groups
load("input_data/observed_occ_pacific_islands.RData") # observed occurrences
load("input_data/occ_numbers_thinned_env_filtered.RData") # Contains names of study species
...
# Create a data frame to store the results
unreal_col_pot_clim_global_comp <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(unreal_col_pot_clim_global_comp) <- c("unrealized_col_pot", "predictor_set", "species")

# Solely use the predictions of ensemble models
islandgroups_results_clim_global_ensemble_comp <- subset(islandgroups_results_clim_global_comp, islandgroups_results_clim_global_comp$algorithm == "Ensemble")


for (sp in study_species) { # Start of the loop over all study species
  
  # Create a data frame to store the results per species
  unreal_col_pot_clim_global_species_comp <- data.frame(matrix(ncol = 7, nrow = 0))
  colnames(unreal_col_pot_clim_global_species_comp) <- c("islandgroup", "a", "b", "c", "d", "predictor_set", "species")
  
  # Subset the prediction data frame by each species
  subset_species <- subset(islandgroups_results_clim_global_ensemble_comp, islandgroups_results_clim_global_ensemble_comp$species == sp)
  ...
  for (i in islandgroup_climate_soil) { # Start of the loop over all island groups
    ...
    # Add the vector containing the results to the data frame
    unreal_col_pot_clim_global_species_comp <- rbind(unreal_col_pot_clim_global_species_comp, results_col_pot)
    
    # Make sure the column names are correct
    colnames(unreal_col_pot_clim_global_species_comp) <- c("islandgroup", "a", "b", "c", "d", "predictor_set", "species")
    
    # Save the data frame for each species
    save(unreal_col_pot_clim_global_species_comp, file = paste0("output_data/unrealized_col_pot/global/clim_comp/unreal_col_pot_clim_global_species_comp_",sp,".RData"))
  ...
  # Calculate the unrealized colonization potential (False Positive Rate) per species
  col_pot_a <- sum(as.numeric(unreal_col_pot_clim_global_species_comp$a)) # a
  col_pot_b <- sum(as.numeric(unreal_col_pot_clim_global_species_comp$b)) # b
  col_pot_c <- sum(as.numeric(unreal_col_pot_clim_global_species_comp$c)) # c
  col_pot_d <- sum(as.numeric(unreal_col_pot_clim_global_species_comp$d)) # d
  ...
  # Add result to the data frame
  unreal_col_pot_clim_global_comp <- rbind(unreal_col_pot_clim_global_comp, results_unreal_col_pot)
...
# Save the results data frame containing the unrealized colonization potential 
# of all study species
save(unreal_col_pot_clim_global_comp, file = "output_data/unrealized_col_pot/global/clim_comp/unreal_col_pot_clim_global_comp.RData")