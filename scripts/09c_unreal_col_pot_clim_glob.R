...
# (a) based on 49 island groups covered by climatic data -----------------------
  ...
  print("Calculate unrealized colonistaion potential based on 49 island groups")
  
  # Load needed objects
  load(paste0("output_data/model_predictions_rev/global/clim/",t,"/islandgroups_results_clim_global.RData")) # Data frame with prediction results of all species based on 49 island groups
  ...
  unreal_col_pot_clim_global <- data.frame(matrix(ncol = 10, nrow = 0))
  colnames(unreal_col_pot_clim_global) <- c("unrealized_col_pot", "TP", "FP", "TP_and_FP", "TN", "algorithm", "predictor_type", "niche", "predictor_set", "species")
  ...
  for (sp in study_species) { # Start of the loop over all study species
    ...
    for (a in algorithm) { # Start the loop over all algorithms
      ...
      unreal_col_pot_clim_global_species <- data.frame(matrix(ncol = 8, nrow = 0))
      colnames(unreal_col_pot_clim_global_species) <- c("islandgroup", "a", "b", "c", "d", "predictor_set", "species", "algorithm")
      ...
      for (i in islandgroup_climate) { # Start of the loop over all island groups
        ...
        if (check_observation == 1 & check_prediction == 1) { results_col_pot <- c(i, 1, 0, 0, 0, 3, sp, a)
        } else if (check_observation == 0 & check_prediction == 0) { results_col_pot <- c(i, 0, 0, 0, 1, 3, sp, a)
        } else if (check_observation == 0 & check_prediction == 1) { results_col_pot <- c(i, 0, 1, 0, 0, 3, sp, a)
        } else if (check_observation == 1 & check_prediction == 0) { results_col_pot <- c(i, 0, 0, 1, 0, 3, sp, a) 
        }
        ...
        # save(unreal_col_pot_clim_global_species, file = paste0("output_data/unrealized_col_pot/global/clim/unreal_col_pot_clim_global_species_",sp,"_",a,".RData"))
      ...
      results_unreal_col_pot <- c(false_positive_rate, true_positives, false_positives, all_positives, true_negatives, a, "clim", "global", 3, sp)
  ...
  save(unreal_col_pot_clim_global, file = paste0("output_data/unrealized_col_pot_rev/global/clim/",t,"/unreal_col_pot_clim_global.RData"))
...
# (b) based on 25 island groups covered by climatic and edaphic data -----------
  ...
  print("Calculate unrealized colonistaion potential based on 25 island groups")
  
  # Load needed objects
  load(paste0("output_data/model_predictions_rev/global/clim_comp/",t,"/islandgroups_results_clim_global_comp.RData")) # Data frame with prediction results of all species based on 25 island groups
  load("input_data/observed_occ_pacific_islands.RData") # observed occurrences
  load("input_data/occ_numbers_thinned_env_filtered.RData") # Contains names of study species
  ...
  # Create a data frame to store the results
  unreal_col_pot_clim_global_comp <- data.frame(matrix(ncol = 10, nrow = 0))
  colnames(unreal_col_pot_clim_global_comp) <- c("unrealized_col_pot", "TP", "FP", "TP_and_FP", "TN", "algorithm", "predictor_type", "niche", "predictor_set", "species")
  ...
  for (sp in study_species) { # Start of the loop over all study species
    ...
    for (a in algorithm) { # Start the loop over all algorithms
      ...
      unreal_col_pot_clim_global_species_comp <- data.frame(matrix(ncol = 8, nrow = 0))
      colnames(unreal_col_pot_clim_global_species_comp) <- c("islandgroup", "a", "b", "c", "d", "predictor_set", "species", "algorithm")
      ...
      for (i in islandgroup_climate_soil) { # Start of the loop over all island groups
        ...
        # Save the data frame for each species
        #save(unreal_col_pot_clim_global_species_comp, file = paste0("output_data/unrealized_col_pot/global/clim_comp/unreal_col_pot_clim_global_species_comp_",sp,"_",a,".RData"))
  ...
  # Save the results data frame containing the unrealized colonization potential 
  # of all study species
  save(unreal_col_pot_clim_global_comp, file = paste0("output_data/unrealized_col_pot_rev/global/clim_comp/",t,"/unreal_col_pot_clim_global_comp.RData"))
