          ...
          for (i in islandgroup_climate) {
            try({
              ...
              env_preds_rf <- data.frame(bio_curr_df_islandgroup[, 1:2], 
                                         rf = rowMeans(sapply(1:10, FUN=function(m) {print(m); predict(m_rf_clim_native[[m]], bio_curr_df_islandgroup[, pred_sel_clim_native], type='response')})))
              ...
              env_preds_brt <- data.frame(bio_curr_df_islandgroup[, 1:2],
                                          brt = rowMeans(sapply(1:10, FUN=function(m) {print(m); predict.gbm(m_brt_clim_native[[m]], bio_curr_df_islandgroup[, pred_sel_clim_native],
                                                                                                             n.trees=m_brt_clim_native[[m]]$gbm.call$best.trees, type='response')})))
              ...
              env_preds_ensemble_bin <- data.frame(bio_curr_df_islandgroup[,1:2], 
                                                   ensemble = ifelse(env_preds_ensemble[,"mean_prob"]>= ensemble_perf_clim_native[t,'thresh'],1,0))
...
# based on each thresholding method
