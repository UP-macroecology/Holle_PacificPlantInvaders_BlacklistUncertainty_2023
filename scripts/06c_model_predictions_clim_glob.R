  ...
  try({
    ...
    for (t in threshold_methods) { # Start the loop over the different thresholding methods
      try({
        ...
        file_exists <- file.exists(paste0(dir, "/output_data/model_predictions_rev/global/clim/",t,"/islandgroups_results_clim_global_spec_",sp,".RData"))
        ...
        if (file_exists == FALSE) { # just continue with model predictions if output 
          ...
          load(paste0(dir, "/output_data/models_rev/global/clim/models_clim_global_",sp,".RData")) # the four different models
          load(paste0(dir, "/output_data/validation_rev/global/clim/validation_clim_global_",sp,".RData")) # validation outcomes
          ...
          comp_perf_clim_global <- get(paste0("comp_perf_clim_global_", t))
          ...
          for (i in islandgroup_climate) {
            try({
              ...
              env_preds_rf <- data.frame(bio_curr_df_islandgroup[, 1:2], 
                                         rf = rowMeans(sapply(1:10, FUN=function(m) {print(m); predict(m_rf_clim_global[[m]], bio_curr_df_islandgroup[, pred_sel_clim_global], type='response')})))
              ...
              env_preds_brt <- data.frame(bio_curr_df_islandgroup[, 1:2],
                                          brt = rowMeans(sapply(1:10, FUN=function(m) {print(m); predict.gbm(m_brt_clim_global[[m]], bio_curr_df_islandgroup[, pred_sel_clim_global],
                                                                                                             n.trees=m_brt_clim_global[[m]]$gbm.call$best.trees, type='response')})))
              ...
              env_preds_ensemble_bin <- data.frame(bio_curr_df_islandgroup[,1:2], 
                                                   ensemble = ifelse(env_preds_ensemble[,"mean_prob"]>= ensemble_perf_clim_global[t,'thresh'],1,0))
          ...
          save(islandgroups_results_clim_global_spec, file = paste0(dir, "/output_data/model_predictions_rev/global/clim/",t,"/islandgroups_results_clim_global_spec_",sp,".RData"))
...
for (t in threshold_methods) { # Start the loop over the three thresholding methods
  try({
    ...
    for (sp in study_species) {
      try({
        ...
        if (file_exists == TRUE) { # just continue with model predictions if output 
          ...
          load(paste0(dir, "/output_data/model_predictions_rev/global/clim/",t,"/islandgroups_results_clim_global_spec_",sp,".RData"))
    ...
    save(islandgroups_results_clim_global, file = paste0(dir, "/output_data/model_predictions_rev/global/clim/",t,"/islandgroups_results_clim_global.RData"))
  ...
  try({
    ...
    for (t in threshold_methods) { # Start the loop over the three thresholding methods
      try({
        ...
        file_exists <- file.exists(paste0(dir, "/output_data/model_predictions_rev/global/clim_comp/",t,"/islandgroups_results_clim_global_spec_comp_",sp,".RData"))
        ...
        if (file_exists == FALSE) { # just continue with new area calculations if output 
          ...
          # Save the results data frame per species
          save(islandgroups_results_clim_global_spec_comp, file = paste0(dir,  "/output_data/model_predictions_rev/global/clim_comp/",t,"/islandgroups_results_clim_global_spec_comp_",sp,".RData"))
...
for (t in threshold_methods) { # Start the loop over the three thresholding methods
  try({
    ...
    for (sp in study_species) {
      try({
        ...
        if (file_exists == TRUE) { # just continue with model predictions if output 
          ...
          # Load the file with predictions results
          load(paste0(dir, "/output_data/model_predictions_rev/global/clim_comp/",t,"/islandgroups_results_clim_global_spec_comp_",sp,".RData"))
    ...
    # Save the data frame containing all prediction results
    save(islandgroups_results_clim_global_comp, file = paste0(dir, "/output_data/model_predictions_rev/global/clim_comp/",t,"/islandgroups_results_clim_global_comp.RData"))
