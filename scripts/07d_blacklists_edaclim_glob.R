...
# Create a data frame to store the results
islandgroups_rank_edaclim_global <- data.frame(matrix(ncol = 6, nrow = 0))
colnames(islandgroups_rank_edaclim_global) <- c("islandgroup", "algorithm", "predictor_type", "niche", "species", "rank")


for (a in algorithm) { # Start the loop over all algorithms
  
  # Subset the data frame  of area calculations by each algorithm
  subset_algorithm <- subset(islandgroups_results_edaclim_global, islandgroups_results_edaclim_global$algorithm == a)
  ...
  for (i in islandgroup_climate_soil) { # Start the loop over all island groups
    ...
    islandgroups_rank_edaclim_global <- rbind(islandgroups_rank_edaclim_global, subset_algorithm_islandgroup_rank)
...
# Create a data frame to store results
results_mean_rank_islandgroups_edaclim_global <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(results_mean_rank_islandgroups_edaclim_global) <- c("rank", "algorithm", "predictor_type", "niche", "species")


for (sp in study_species) { # Start the loop over all species
  
  # Subset the data frame by species
  subset_species <- subset(islandgroups_rank_edaclim_global, islandgroups_rank_edaclim_global$species == sp)
  ...
  for (a in algorithm) { # Start the loop over all algorithms
    ...
    results_mean_rank <- c(mean_rank, a, "edaclim", "global", sp)
    
    # Add the results vector to the data frame
    results_mean_rank_islandgroups_edaclim_global <- rbind(results_mean_rank_islandgroups_edaclim_global, results_mean_rank)
...
# Save the resulting data frame
save(results_mean_rank_islandgroups_edaclim_global, file = "output_data/blacklists/global/edaclim/results_mean_rank_islandgroups_edaclim_global.RData")
