...
foreach(sp = study_species, .packages = c("mgcv", "randomForest", "gbm", "dismo", "PresenceAbsence", "ecospat")) %dopar% { # Start the loop over all species
  try({
    ...
    prog_log_file <- file(file.path("output_data", "models", "model_fitting_validation_progress_clim_global.txt"), open = "at") # write console output here
    ...
    # with models does not exist yet
      ...
      save(m_glm_clim_global, m_gam_clim_global, m_rf_clim_global, m_brt_clim_global, species_occ_clim_global, weights, file = paste0("output_data/models/global/clim/models_clim_global_",sp,".RData"))
...
# 2. Model validation ---------------------------------------------------------- 
    ...
    # with validation files does not exist yet
      ...
      load(paste0("output_data/models/global/clim/models_clim_global_",sp,".RData"))
      load(paste0("output_data/variable_selection/global/clim/pred_sel_clim_global_",sp,".RData"))
      ...
      save(m_glm_preds_cv, m_gam_preds_cv, m_brt_preds_cv, m_rf_preds_cv, preds_cv_ens_clim_global, crossval_folds, 
           ...
           ensemble_perf_clim_global,  file = paste0("output_data/validation/global/clim/validation_clim_global_",sp,".RData"))
