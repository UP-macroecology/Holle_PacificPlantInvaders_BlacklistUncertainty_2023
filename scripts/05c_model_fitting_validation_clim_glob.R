...
for (sp in study_species) { # Start the loop over all species
  try({
    ...
    file_exists_models <- file.exists(paste0("output_data/models/global/clim/models_clim_global_",sp,".RData"))
    ...
    # with models does not exist yet
      ...
      species_occ_clim_global$abs_index[species_occ_clim_global$occ!=1] <- sample(1:10,sum(species_occ_clim_global$occ!=1), replace=T)
      ...
      m_rf_clim_global <- lapply(1:10, FUN=function(i) {randomForest(x = species_occ_clim_global[is.na(species_occ_clim_global$abs_index) | species_occ_clim_global$abs_index == i , pred_sel_clim_global],
                                                                     y = species_occ_clim_global[is.na(species_occ_clim_global$abs_index) | species_occ_clim_global$abs_index == i,]$occ, 
      ...
      m_brt_clim_global <- lapply(1:10, FUN=function(i) {
        ...
        LR <- 0.01;
        while(opt.LR){
          m.brt <- try(gbm.step(data = species_occ_clim_global[is.na(species_occ_clim_global$abs_index) | species_occ_clim_global$abs_index==i,], gbm.x = pred_sel_clim_global, gbm.y = "occ", family = 'bernoulli', tree.complexity = 2, bag.fraction = 0.75, learning.rate = LR, verbose=F, plot.main=F))
...
# 2. Model validation ---------------------------------------------------------- 
    ...
    file_exists_validation <- file.exists(paste0("output_data/validation/global/clim/validation_clim_global_",sp,".RData"))
